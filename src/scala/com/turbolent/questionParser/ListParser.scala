...
object ListParser extends BaseParser {
    ...
    (opt(Determiner) ~ rep(AnyAdjective) ~ Nouns) ^^ {
      case optDeterminer ~ adjectives ~ nouns =>
        optDeterminer
          ...
          .getOrElse(ast.NamedValue(adjectives ++ nouns))
    }
  ...
  lazy val Values: PackratParser[Value] =
    commaOrAndList(Value, ast.AndValue, ast.OrValue, andOptional = false)
  ...
  lazy val Filters: PackratParser[Filter] =
    commaOrAndList(Filter, ast.AndFilter, ast.OrFilter, andOptional = false)
  ...
  lazy val InversePropertySuffix
    : PackratParser[(List[Token], Filter) => InversePropertyWithFilter] =
    (Verbs ~ opt(Particle | (Preposition <~ not(NamedValue)))) ^^ {
  ...
  lazy val QueryRelationships: PackratParser[ast.Query] =
    chainl1(Queries, Possessive ^^ { sep => (a: ast.Query, b: ast.Query) =>
  ...
  lazy val QueryProperties: PackratParser[Query => QueryWithProperty] =
    Properties ^^ { property => (query: ast.Query) =>
  ...
  lazy val FullQuery: PackratParser[ast.Query] =
    ((QueryRelationships <~ opt(WhDeterminer | "who")) ~ opt(
      QueryProperties ||| QueryRelationship)) ^^ {
      case query ~ None =>
        query
