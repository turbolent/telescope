  ...
  val factories: mutable.Map[(String, String), ContextfulEdgeFactory] =
    mutable.Map(
      ...
      ("locate", "in") -> { (node, context, env) =>
        out(P.isLocatedIn, node)
          .or(out(P.country, node))
          .or(out(P.hasHeadquartersLocation, node))
      },
...
trait ValuePropertyEdgeFactory
    extends questionCompiler.ValuePropertyEdgeFactory[NodeLabel,
                                                      EdgeLabel,
                                                      WikidataEnvironment] {

  def makeValuePropertyEdge(propertyName: Seq[Token],
                            node: WikidataNode,
                            context: EdgeContext,
